# Digital-Analytics-Ecommerce
Portfolio project analyzing conversion funnels, customer behavior and campaign performance.

## Overview
This project presents an end-to-end digital analytics solution for e-commerce growth analysis using SQL, Python, Power BI and Streamlit.

---
## Project Components

### SQL Queries
Primary SQL analysis scripts:  
[View SQL Queries](./SQL%20CODES.sql)

---

## Python Analysis

### Analysis Deck Python Codes
[View Notebook](./Python/Analysis_Deck_python_codes.ipynb)

### Investor Pitch Python Codes
[View Notebook](./Python/Investior_Pitch%20codes.ipynb)

---

## Dashboard Screenshots

### Business Performance Overview
![Business Performance](./Images/Business%20Performance%20Overview.png)

### Product Performance & Profitability
![Product Performance](./Images/Product%20Performance%20Profitability.png)

### Website Traffic & Engagement Dashboard
![Traffic Dashboard](./Images/Website%20Traffic%20Engagement%20Performance%20Dashboard.png)

---

## Live Applications

### Streamlit App
Access the app here:  
[Open Streamlit App](https://digianalysis-qbvsufj7csxrgl6mcviulg.streamlit.app)

Demo Credentials (for reviewer access):  
Email: admin@gmail.com  
Password: scalar@123

---

### Power BI Dashboard
View interactive dashboard here:  
[Open Power BI Dashboard](https://app.powerbi.com/groups/me/reports/b8d45889-0600-4eaa-b810-925f1fa7b806/697bc48fc608179be4be?experience=power-bi)

---

## Collaboration & Project Coordination
This project was delivered in a team environment with cross-functional collaboration.

My coordination contributions included:

- Allocating tasks among team members
- Aligning SQL, Python, dashboard and app deliverables
- Tracking milestones and review progress
- Managing project discussions and decision-making
- Coordinating stakeholder presentations and feedback incorporation

---

## My Contributions
- SQL-based analytics
- Python analytical workflows
- Business insight generation
- Dashboard interpretation
- Stakeholder presentations
- Team coordination
